...
##openCV ??
###\lmcst\utils\cv2\x86\cv2.pyd
from common.contant import LMCFT_FOLDER_RESULT, LMCFT_FOLDER_FAIL, LMCFT_EXEC_HISTORY
from common.deviceInfo import getModelNameFromDevice
...
APK_DIRECTORY = '\\\\TDDS1\\pipeline\\tphone_android\\'
...
if not os.path.isdir(APK_DIRECTORY):
   raw_input(APK_DIRECTORY + ' is not found![enter]')
   exit(1)
...
home = expanduser("~")
emulator_console_auth_token=None
with open(home + "\\.emulator_console_auth_token") as f:
   emulator_console_auth_token = f.readline()

if emulator_console_auth_token == None:
   print("%s:%s" % ("emulator_console_auth_token", emulator_console_auth_token))
   raw_input('.emulator_console_auth_token is not found![enter]')
   exit(1)
...
def loadDataSet(datatype):
   ...
   if datatype == 'excel':
      worksheetName = unicode('워크시트 익스포트', 'utf-8')
      #worksheetName = '워크시트 익스포트'
      print (worksheetName)
      print ("type call start")
      print (type(worksheetName))
      print ("type call end")
      SHEETS = [
#         ['20170808_QITP-2116_T114.xlsx', '워크시트 익스포트'.encode('euc-kr'), 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', unicode('워크시트 익스포트').encode('euc-kr'), 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', '워크시트 익스포트', 'A', 'B', 2, 1001],
#          ['20170808_QITP-2116_T114.xlsx', worksheetName.encode('euc-kr'), 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', worksheetName, 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', worksheetName.encode('utf-8'), 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', u'워크시트 익스포트', 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', worksheetName.encode('latin-1'), 'A', 'B', 2, 1001],
#         ['20170808_QITP-2116_T114.xlsx', worksheetName.encode('cp949'), 'A', 'B', 2, 1001],
         [u'20171023_QITP-2116_T114_739개.xlsx', worksheetName.encode('cp949'), 'A', 'B', 2, 740],
      ]

      SHEET = SHEETS[0]
      wb = load_workbook(filename=SHEET[0])
      #한글 워크 시트 이름을 읽을 방법을 찾지 못해 sheet_ranges = wb[SHEET[1]] 대신해 아래와 같이 수정
      sheet_ranges = wb.active#sheet_ranges = wb[SHEET[1]]
...
#   os.system("taskkill /F /IM adb.exe")
   ...
   print ('adb logcat -c')
   if logFileName == None:
      FAILSARCHIVE = LMCFT_FOLDER_FAIL + myName.split('.')[0] + "\\" + getModelNameFromDevice(DEVICE_ID) + "\\" + START_TIME
      logFileName = FAILSARCHIVE + "\\" + \
                    TARGET_APP + "_" + str(reVisionCount) + "_" + str(commitHashCode) + "_" + hostname + "_" + START_TIME + "_" + str(during_hours) + ".log"
      mkdirs(logFileName)
...
MODEL =  getModelNameFromDevice(DEVICE_ID)
resultS['MODEL'] = MODEL
...
FAILSARCHIVE = LMCFT_FOLDER_FAIL + myName.split('.')[0] + "\\" + MODEL + "\\" + START_TIME
if logFileName == None:
   logFileName = FAILSARCHIVE+ "\\" + \
                  TARGET_APP + "_" + str(reVisionCount) + "_" + str(commitHashCode) + "_" + hostname  + "_" + START_TIME + "_" + str(during_hours) + ".log"
mkdirs(logFileName)
...
failListFileName = FAILSARCHIVE + "\\" + \
                    "FailList" + "_" + TARGET_APP + "_" + str(reVisionCount) + "_" + str(commitHashCode) + "_" + hostname  + "_" + START_TIME + "_" + str(during_hours) + ".log"
#resultFileName = "Result" + "_" + myName.split('.')[0] + "_" + TARGET_APP + "_" + hostname  + ".log"
resultFileName = LMCFT_FOLDER_RESULT + \
                 "Result" + "_" + myName.split('.')[0] + ".log"
